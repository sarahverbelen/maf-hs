...
data Span = Span { filename :: String, line :: Int, column :: Int } | NoSpan deriving (Show, Ord, Generic) 

instance Eq Span where 
   NoSpan == _ = True
   _ == NoSpan = True 
   (Span fileA lineA colA) == (Span fileB lineB colB) = (fileA == fileB) && (lineA == lineB) && (colA == colB)
